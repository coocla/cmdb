...
class GetHostConfig(object):
    ...
    @property
    def config(self):
        return base64.encodestring(self.data).replace('\n', '').strip()

    def pre_check(self):
        if not self.host.control:
            return
    ...
    def initial(self):
        ...
        agentdata = agentData(
                rabbitmq_host=self.host.control.rabbitmq_host,
                rabbitmq_port=self.host.control.rabbitmq_port,
                rabbitmq_ssl=self.host.control.rabbitmq_ssl,
                rabbitmq_down_exchange=self.host.control.rabbitmq_down_exchange,
                rabbitmq_down_queue=self.host.control.rabbitmq_down_exchange,
                rabbitmq_up_exchange=self.host.control.rabbitmq_up_exchange,
                rabbitmq_up_queue=self.host.control.rabbitmq_up_exchange,
                rabbitmq_vhost=self.host.control.rabbitmq_vhost,
                queue_username=self.host.control.queue_username,
                queue_password=self.host.control.queue_password,
                agent_id=self.host.uuid)
        self.data = agentdata.toJSON()
